...
const Edit = () => {
  ...
  const { detail, getDetail, edit, editLoading, getStatics, staticsLoading, statics } =
  ...
  useEffect(() => {
    setParams(() => ({
      introStatement: detail?.introStatement,
      theoryGrade: detail?.theoryGrade,
      practiceGrade: detail?.practiceGrade,
      qualificationDetails: detail?.qualificationDetails,
      issuanceBasis: detail?.issuanceBasis,
      issuingBody: detail?.issuingBody,
      city: detail?.city,
      certificatesName: detail?.certificatesName,
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      const data = { ...params }
      data['status'] = params.status?.value || 0
      edit(id, data)
        .then((res) => {
          if (res?.success) {
